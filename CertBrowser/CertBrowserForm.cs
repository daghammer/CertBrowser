...
/*
 * Cert browser for bruk i testmiljø og prod for Serifikater fra Buypass og Commfides
 * 
 * V1.3, DH : Lagt til OU felt som søkealternativ - aktuell for å finne apotek
 * 
 */
...
{
    public partial class CertBrowserForm : Form
    {
...
//               filterSerial = "(serialNumber=" + tbSerial.Text.Trim() + ")";
            ...
            String filterOrgUnit = "";
            if (tbOU.Text.Trim().Length > 0)
            {
                filterOrgUnit = "(OU=" + tbOU.Text.Trim() + ")";
                fieldCount++;
            }
            if (fieldCount == 0) filter = "CN=*";
            if (fieldCount >= 1) filter = filterCommonName + filterOrgno + filterSerial + filerOrgSerial + filterOrgUnit; // Combine the filters
        ...
        private void listBoxCert_DoubleClick(object sender, EventArgs e)
        {
            ...
            try
            {
                if (!c.Verify())
                {
                    MessageBox.Show("dette sertifikatet er ikke gyldig!");
                }

            }
            catch ( System.Security.Cryptography.CryptographicException cExpt )
            {
                 MessageBox.Show( cExpt.Message,  "Kunne ikke verifisere sertifikatet");
            }
        ...
        {
            String resultText = "";
            String number = tbSerial.Text.Trim().Replace(" ", "");
